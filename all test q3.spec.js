    ...
    async function creationSubstation(substation) {
        for (const field of substation) {
            ...
            if (field.type === 'text') {
                ...
                const plusButton = await driver.findElement(By.xpath('//*[@id="for_fk_substation_mrid"]/div[2]/button[2]')).click();
                const searchButton = await driver.findElement(By.xpath('//*[@id="period_section_fk_substation_mrid"]/div/span[1]')).click();
    ...
    it('отправка заполненной формы добавления подстанции', async function () {
        ...
        const substation = [
            ...
            {
                id: 'fk_substation_mrid_0', //Выберите сети 
            ...
            },
            {
                id: 'code_tm', //Код ТМ 
                type: 'text',
                text: ''+ Math.floor(Math.random() * 200) + 1,
    ...
    let btnAddFreeForm = await driver.findElement(By.xpath('//*[@id="lines-list_filter"]/button')).click();
        ...
        const line = [
            ...
            },
            {
       	   ...
       	   {
                id: 'fk_line_mrid_0', //Выберите сети 
                type: 'periodical_section',
                text: 'Московский РЭС',
                error: '6-Поле "Выберите сети" обязательно. ',
            },
    ...
    async function addName(nameAdd) {
        for (const field of nameAdd) {
            ...
            if (field.type === 'text') {
                ...
                const errorDiv = await driver.findElement(By.id('error_div'));
                const errorMessage = await errorDiv.getText();
                assert.strictEqual(errorMessage, nameAdd[0].error, `Несоответствие сообщения об ошибке для поля с: ${newNameAdd[0].id}`);
            }
        }
        const btnSave = await driver.findElement(By.id('btn_save')).click();
    }
